...
void main()
{
    ...
    vec3 M = vec3(u_mouse.xy/u_resolution.xy, 0.0);
    ...
    // 将基于 uv（按 y 归一）的法线偏移 n 转换到 UV（0-1）坐标系，避免横向拉伸导致的偏移
    float aspect = u_resolution.x / u_resolution.y;
    vec2 nUV = n / vec2(aspect, 1.0);
    // 确保纹理坐标在有效范围内
    vec2 texCoord = clamp(UV + nUV, 0.0, 1.0);
    vec3 col = textureLod(iChannel0, texCoord, focus).rgb;
